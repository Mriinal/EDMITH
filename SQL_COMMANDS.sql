-- ==============================================================================
-- EDMITH COMPLETE APPLICATION DATABASE REBUILD (SQL_COMMANDS.sql)
-- ==============================================================================
-- Designed by Reverse-Engineering the Complete EDMITH Frontend (HTML/JS/CSS).
-- Authoritative, self-contained, idempotent rebuild for Supabase/PostgreSQL.
-- 
-- SAFE EXECUTION NOTICE:
-- 1. Drops ONLY EDMITH application objects in public schema.
-- 2. Preserves auth.users and Supabase internal infrastructure.
-- 3. Automatically backfills public.users from auth.users.
-- 4. Full RLS policies matched to frontend queries, inserts, and updates.
-- ==============================================================================
...
-- ==============================================================================
-- 2. SAFE DESTRUCTIVE RESET (EDMITH APPLICATION OBJECTS ONLY)
-- ==============================================================================

-- 2.1 Drop Views
DROP VIEW IF EXISTS public.v_sql_leaderboard CASCADE;

-- 2.2 Drop RPC Functions
DROP FUNCTION IF EXISTS public.get_sql_leaderboard(INTEGER, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_leaderboard(TEXT, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_leaderboard(INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_leaderboard(TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_leaderboard() CASCADE;
DROP FUNCTION IF EXISTS public.get_user_sql_leaderboard_position(UUID) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_exam_leaderboard(TEXT, TEXT, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS public.get_sql_exam_user_position(TEXT, TEXT) CASCADE;

-- 2.3 Drop Triggers and Trigger Functions
...
-- 2.4 Drop Dependent Child Tables (Answers)
DROP TABLE IF EXISTS public.test_attempt_answers CASCADE;
DROP TABLE IF EXISTS public.sql_exam_answers CASCADE;

-- 2.5 Drop Assessment Attempts & Progress Tracking Tables
DROP TABLE IF EXISTS public.test_attempts CASCADE;
DROP TABLE IF EXISTS public.sql_exam_attempts CASCADE;
DROP TABLE IF EXISTS public.lesson_progress CASCADE;
DROP TABLE IF EXISTS public.course_progress CASCADE;

-- 2.6 Drop Content Definitions & Question Bank
DROP TABLE IF EXISTS public.test_questions CASCADE;
DROP TABLE IF EXISTS public.test_rounds CASCADE;
DROP TABLE IF EXISTS public.subjects CASCADE;

-- 2.7 Drop User Profiles Table
...
-- 3. TYPES & DOMAINS
-- ==============================================================================
-- Standardized text types with check constraints are used to ensure direct
-- compatibility with Supabase JavaScript payloads and avoid strict enum migrations.


-- ==============================================================================
-- 4. TABLES (STRUCTURE & PRIMARY KEYS)
-- ==============================================================================

-- 4.1 Public Users Profile Table
-- Holds application profile data synced with auth.users and edited via edit-profile.html.
CREATE TABLE public.users (
    ...
    phone               VARCHAR(20),
    country             VARCHAR(60)     NOT NULL DEFAULT 'Other',
...
-- 4.3 Test Rounds Definitions Table (MCQ & Coding round structure)
CREATE TABLE public.test_rounds (
    ...
    is_active           BOOLEAN         NOT NULL DEFAULT TRUE,
    CONSTRAINT unique_subject_round UNIQUE (subject_slug, test_type, difficulty, round_number)
);

-- 4.4 Test Question Bank Table (300 MCQs & 120 Coding Questions)
...
-- 4.5 Course Progress Tracking Table (Syncs with javascript/script.js)
...
-- 4.6 Lesson Progress Tracking Table (Syncs with javascript/script.js)
...
-- 4.7 Test Attempts Table (Unified Assessment Attempts for MCQ & Coding)
-- Every completed attempt is counted individually (1 MCQ + 2 Coding = 3 tests).
...
-- 4.8 Test Attempt Answers Table
...
-- 4.9 Legacy SQL Exam Attempts Table (Backward Compatibility for existing attempts)
CREATE TABLE public.sql_exam_attempts (
    id                  UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id             UUID            NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- 4.10 Legacy SQL Exam Answers Table (Backward Compatibility)
CREATE TABLE public.sql_exam_answers (
    id                  UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE UNIQUE INDEX idx_users_email_lower    ON public.users (LOWER(TRIM(email)));
CREATE UNIQUE INDEX idx_users_username_lower ON public.users (LOWER(TRIM(username)));
...
CREATE INDEX idx_lesson_progress_lookup      ON public.lesson_progress(course_id, lesson_id);

-- Test questions bank indexes
CREATE INDEX idx_test_questions_filter       ON public.test_questions (subject_slug, test_type, difficulty, is_active);

-- Test attempts & Leaderboard indexes
CREATE INDEX idx_test_attempts_user_sub      ON public.test_attempts (user_id, subject_slug, status, completed_at DESC);
CREATE INDEX idx_test_attempts_leaderboard   ON public.test_attempts (subject_slug, status, score DESC, completed_at ASC);
CREATE INDEX idx_test_attempt_answers_att    ON public.test_attempt_answers (attempt_id);

-- Legacy attempts indexes
CREATE INDEX idx_sql_exam_attempts_user_lvl  ON public.sql_exam_attempts(user_id, exam_level, completed_at DESC);
CREATE INDEX idx_sql_exam_answers_att        ON public.sql_exam_answers(attempt_id);


-- ==============================================================================
-- 6. TRIGGER FUNCTIONS & AUTOMATIC USER PROFILE SYNCHRONIZATION
-- ==============================================================================

-- Master Trigger Function: Auto-sync auth.users to public.users
-- Fires on every sign-up, sign-in metadata change, or updateUser call.
...
-- ==============================================================================
-- 8. ROW LEVEL SECURITY POLICIES (MATCHING FRONTEND ACTIONS)
-- ==============================================================================

-- 8.1 public.users Policies
-- Allows anonymous and authenticated users to check username availability and sign in with username.
CREATE POLICY "Anyone can read usernames for auth and profiles"
...
-- Authenticated users can update their own profile row via edit-profile.html.
...
-- 8.2 public.course_progress Policies
...
-- 8.3 public.lesson_progress Policies
...
-- 8.4 public.subjects Policies (Read-only for all active courses)
CREATE POLICY "Anyone can view active subjects"
    ON public.subjects FOR SELECT
    USING (is_active = true);

-- 8.5 public.test_rounds Policies
CREATE POLICY "Anyone can view active test rounds"
    ON public.test_rounds FOR SELECT
    USING (is_active = true);

-- 8.6 public.test_questions Policies
CREATE POLICY "Anyone can read active test questions"
    ON public.test_questions FOR SELECT
    USING (is_active = true);

-- 8.7 public.test_attempts Policies
-- Authenticated users can view their own attempts in profile dashboard.
CREATE POLICY "Users can view own test attempts"
    ON public.test_attempts FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

-- Anyone can view completed attempts for leaderboard ranking.
CREATE POLICY "Anyone can view completed attempts for leaderboard"
    ON public.test_attempts FOR SELECT
    USING (status = 'completed');

-- Authenticated users can insert their own attempts from tests.
...
-- 8.8 public.test_attempt_answers Policies
...
-- 8.9 public.sql_exam_attempts Policies (Legacy Compatibility)
CREATE POLICY "Users can view own legacy attempts"
    ON public.sql_exam_attempts FOR SELECT
    TO authenticated
    USING (auth.uid() = user_id);

CREATE POLICY "Anyone can view legacy attempts for leaderboard"
    ON public.sql_exam_attempts FOR SELECT
    USING (true);

CREATE POLICY "Users can insert own legacy attempts"
    ON public.sql_exam_attempts FOR INSERT
    TO authenticated
    WITH CHECK (auth.uid() = user_id);

-- 8.10 public.sql_exam_answers Policies (Legacy Compatibility)
CREATE POLICY "Users can view own legacy answers"
    ON public.sql_exam_answers FOR SELECT
    TO authenticated
    USING (EXISTS (SELECT 1 FROM public.sql_exam_attempts a WHERE a.id = attempt_id AND a.user_id = auth.uid()));

CREATE POLICY "Users can insert own legacy answers"
    ON public.sql_exam_answers FOR INSERT
    TO authenticated
    WITH CHECK (EXISTS (SELECT 1 FROM public.sql_exam_attempts a WHERE a.id = attempt_id AND a.user_id = auth.uid()));


-- ==============================================================================
-- 9. VIEWS (UNIFIED SINGLE SQL LEADERBOARD)
-- ==============================================================================
-- Combines MCQ and Coding attempts for all users.
-- Accurately counts every test: 1 MCQ + 2 Coding = 3 tests.
-- Zero scores are valid attempts and are ranked properly.
-- Resolves display name from public.users with auth.users fallback.
...
user_aggregates AS (
    ...
    FROM all_attempts a
    LEFT JOIN public.users p ON p.auth_user_id = a.user_id
...
-- ==============================================================================
-- 10. RPC FUNCTIONS & STORED PROCEDURES (CALLED BY JAVASCRIPT)
-- ==============================================================================

-- 10.1 Primary Leaderboard RPC: get_sql_leaderboard
-- Called by javascript/sql-leaderboard.js: client.rpc('get_sql_leaderboard', { p_limit: limit })
...
-- Overload supporting difficulty first if legacy client calls with positional parameters
CREATE OR REPLACE FUNCTION public.get_sql_leaderboard(
    p_difficulty TEXT,
    p_limit INTEGER DEFAULT 100
)
...
-- 10.3 Legacy Leaderboard RPCs (sql_exam_tables.sql backward compatibility)
...
AS $$
    WITH completed AS (
        ...
        SELECT b.*,
               COALESCE(NULLIF(TRIM(CONCAT_WS(' ', u.first_name, u.last_name)), ''), NULLIF(u.username, ''), 'EDMITH User') AS display_name,
               COUNT(*) OVER (PARTITION BY b.user_id, b.exam_level) AS attempt_count_dummy
        FROM best b
        LEFT JOIN public.users u ON u.auth_user_id = b.user_id
    ), ranked AS (
        SELECT n.*,
...
AS $$
    SELECT 
        ...
        l.completed_at,
        l.attempt_count
    FROM public.get_sql_exam_leaderboard(p_exam_type, p_exam_level, 500) l
...
GRANT ALL ON public.course_progress TO authenticated, service_role;

GRANT SELECT ON public.lesson_progress TO anon;
GRANT ALL ON public.lesson_progress TO authenticated, service_role;
...
GRANT SELECT ON public.test_attempts TO anon;
GRANT ALL ON public.test_attempts TO authenticated, service_role;

GRANT SELECT ON public.test_attempt_answers TO anon;
GRANT ALL ON public.test_attempt_answers TO authenticated, service_role;

GRANT SELECT ON public.sql_exam_attempts TO anon;
GRANT ALL ON public.sql_exam_attempts TO authenticated, service_role;

GRANT SELECT ON public.sql_exam_answers TO anon;
GRANT ALL ON public.sql_exam_answers TO authenticated, service_role;

-- Views & RPCs permissions
GRANT SELECT ON public.v_sql_leaderboard TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_sql_leaderboard(INTEGER, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_sql_leaderboard(TEXT, INTEGER) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_user_sql_leaderboard_position(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_sql_exam_leaderboard(TEXT, TEXT, INTEGER) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_sql_exam_user_position(TEXT, TEXT) TO anon, authenticated;


-- ==============================================================================
-- 12. SEED DATA & USER BACKFILL
-- ==============================================================================
...
-- 12.2 Seed Test Rounds (5 rounds each for Easy, Medium, Hard in MCQ and Coding)
...
-- 12.3 Backfill Existing auth.users into public.users
-- Guarantees all existing accounts have their public profile created immediately.
INSERT INTO public.users (auth_user_id, email, username, first_name, last_name, country, learning_goal, updated_at)
SELECT 
    id,
    email,
    COALESCE(raw_user_meta_data->>'username', SPLIT_PART(email, '@', 1)),
    COALESCE(raw_user_meta_data->>'first_name', 'Learner'),
    raw_user_meta_data->>'last_name',
    COALESCE(raw_user_meta_data->>'country', 'Other'),
    raw_user_meta_data->>'learning_goal',
    NOW()
FROM auth.users
ON CONFLICT (email) DO UPDATE SET
    auth_user_id = EXCLUDED.auth_user_id,
    ...
    learning_goal = COALESCE(public.users.learning_goal, EXCLUDED.learning_goal);
...
-- ==============================================================================
-- 13. NON-DESTRUCTIVE VALIDATION QUERIES
-- Run these queries after executing the script to verify the rebuild.
-- ==============================================================================

-- 13.1 Check all EDMITH tables exist in public schema
SELECT table_name
...
-- 13.2 Verify Row Level Security is ENABLED on all EDMITH tables
SELECT tablename, rowsecurity
FROM pg_tables
WHERE schemaname = 'public'
ORDER BY tablename;

-- 13.3 Verify RLS Policies
...
-- 13.5 Verify Functions & RPCs
SELECT routine_name, routine_type, security_type
FROM information_schema.routines
WHERE routine_schema = 'public'
  AND routine_name IN ('get_sql_leaderboard', 'get_user_sql_leaderboard_position', 'handle_user_profile_sync')
ORDER BY routine_name;

-- 13.6 Verify Triggers on auth.users
SELECT trigger_name, event_manipulation, event_object_table, action_statement
FROM information_schema.triggers
WHERE event_object_table = 'users'
ORDER BY trigger_name;

-- 13.7 Verify Question Bank Counts
SELECT test_type, difficulty, count(*) AS question_count
FROM public.test_questions
GROUP BY test_type, difficulty
ORDER BY test_type, difficulty;

-- 13.8 Preview Leaderboard
SELECT * FROM public.v_sql_leaderboard LIMIT 10;


-- ==============================================================================
-- 14. UPDATES & ENHANCEMENTS (EDMITH PROFILE & SECURITY)
-- ==============================================================================

-- updated code for edit profile UPDATE policy
...
CREATE POLICY "Users can update own profile"
    ...
    WITH CHECK (
        (auth.uid() = auth_user_id OR LOWER(email) = LOWER(auth.jwt() ->> 'email'))
        AND (email IS NULL OR LOWER(email) = LOWER(auth.jwt() ->> 'email'))
    );


-- updated code for edit profile INSERT policy
...
-- updated code for immutable email protection trigger

CREATE OR REPLACE FUNCTION public.prevent_user_email_modification()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF OLD.email IS NOT NULL AND NEW.email IS DISTINCT FROM OLD.email THEN
        RAISE EXCEPTION 'Email address is immutable and cannot be changed after sign-up.';
    END IF;
    RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_prevent_user_email_update ON public.users;
CREATE TRIGGER trg_prevent_user_email_update
    BEFORE UPDATE ON public.users
    FOR EACH ROW
    EXECUTE FUNCTION public.prevent_user_email_modification();


-- updated code for automatic course completion after progress exceeds 90%

CREATE OR REPLACE FUNCTION public.handle_course_progress_auto_complete()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    IF NEW.progress_percentage > 90 AND NEW.completed_at IS NULL THEN
...
DROP TRIGGER IF EXISTS trg_auto_complete_course_progress ON public.course_progress;
CREATE TRIGGER trg_auto_complete_course_progress
    BEFORE INSERT OR UPDATE ON public.course_progress
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_course_progress_auto_complete();


-- updated code for coding test questions starter code cleanup

UPDATE public.test_questions
SET starter_code = '-- Write your code here'
WHERE test_type = 'coding';
